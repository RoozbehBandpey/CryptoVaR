...
def fetch_tick(group, pdf):
  tick = group[0]
  try:
    raw = yf.download(tick, start="2017-01-01", end=end_date)[['Open', 'High', 'Low', 'Close', 'Volume']]
...
# MAGIC %md
# MAGIC ## `STEP4` Compute daily log return
# MAGIC Let's log transform the returns for our stocks, as well as those for our market indicators.
...
# MAGIC Let's see the historical operations that result in current versions, as well as query past versions of our data.

# COMMAND ----------

# MAGIC %sql
# MAGIC DESCRIBE HISTORY market_db.market_return

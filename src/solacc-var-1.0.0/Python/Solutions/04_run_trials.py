...
# ANSWER
mlflow.set_experiment("/Users/odl_user_251397@databrickslabs.onmicrosoft.com/solacc-var-1.0.0/Python/Solutions/04_run_trials")
...
def logTableVersion(tablename:str, v:int):
  try:
    table_version = sql("DESCRIBE HISTORY " + tablename).select("version").collect()[0][0]
    mlflow.log_param(f"v{v}.{tablename}", table_version) 
  except:
    print(f"{tablename} is not a Delta Lake table")
...
def fetch_tick(group, pdf):
  ...
  end_date = group[2]
  try:
    raw = yf.download(tick, start=start_date, end=end_date)[['Open', 'High', 'Low', 'Close', 'Volume']]
...
end_date = run_date
start_date = spark.sql(f"SELECT DATE_FORMAT(DATE_SUB('{run_date}', 30), 'yyyy-MM-dd')").collect()[0][0]
...
(spark
  ...
  .groupBy("ticker", F.lit(start_date), F.lit(end_date))
  .applyInPandas(fetch_tick, schema=schema)
  .createOrReplaceTempView("stock_new"))

spark.sql(f"""
  MERGE INTO {stock_table} a
  USING stock_new b
...
for tick in factors.keys():    
    raw = raw = yf.download(tick, start=start_date, end=end_date)
    # fill in missing business days
    idx = pd.date_range(raw.index.min(), raw.index.max(), freq='B')
    # use last observation carried forward for missing value
...
(spark.createDataFrame(factors_df)
  .createOrReplaceTempView("market_new"))

spark.sql(f"""
  MERGE INTO {market_table} a
  USING market_new b
  ON a.date=b.date
  WHEN NOT MATCHED
    THEN INSERT *""")
...
(x_train.groupBy('ticker')
  .applyInPandas(train_model, schema=schema)
  .select(F.to_date(F.lit(run_date)).alias("run_date"), F.lit(run_uuid).alias("run_uuid"), "*")
  .write
  .format("delta")
  .mode("append")
  .saveAsTable("model_params"))
...
def loadModels(table_name:str, run_uuid:str) -> pyspark.broadcast.Broadcast:
  models_pdf = spark.table("model_params").select("ticker", "weights").filter(F.col("run_uuid")==run_uuid).toPandas()
  models = {}
  for model in np.array(models_pdf):
...
def compute_volatility(table_name: str, run_date: str):
  f_ret_pdf = spark.table(table_name).filter((F.col("date") <= run_date) & (F.col("date") > F.date_sub(F.lit(run_date), 90))).toPandas()
...
if "run_uuid" not in spark.table(trial_table).columns:
  spark.sql(f"ALTER TABLE {trial_table} ADD COLUMNS (run_uuid STRING AFTER run_date)")
...
seed_init = int(datetime.strptime(run_date, '%Y-%m-%d').timestamp())
...
# Compute volatility
f_ret_avg, f_ret_cov = compute_volatility(market_table, run_date)
...
(mc_df
  ...
  .select('run_date', F.lit(run_uuid).alias("run_uuid"), 'ticker', 'seed', 'trial', 'industry', 'country')
  .write
